...
with open('Dataset.csv','w',newline='') as file:
        ...
        writer.writerow(['Name','Fries','Wings','Burger','Pizza','Ice Cream','Drinks','Total'])
root = Tk()
...
def Ref():
    ...
    costoffries = cof*25
    costofwings = colfries*40
    costofburger = cob*35
    costoffilet = cofi*50
    costoficecream = coic*50
    costofdrinks = codr*35

    costofmeal = "Rs.",str('%.2f'% (costoffries +  costofwings + costofburger + costoffilet + costoficecream + costofdrinks))
    PayTax=((costoffries +  costofwings + costofburger + costoffilet +  costoficecream + costofdrinks)*0.33)
    Totalcost=(costoffries +  costofwings + costofburger + costoffilet  + costoficecream + costofdrinks)
    Ser_Charge=((costoffries +  costofwings + costofburger + costoffilet + costoficecream + costofdrinks)/99)
    Service="Rs.",str('%.2f'% Ser_Charge)
    OverAllCost="Rs.",str( '%.2f'%(PayTax + Totalcost + Ser_Charge))
    PaidTax="Rs.",str('%.2f'% PayTax)
    SubCost="Rs.",str('%.2f'% (PayTax + Ser_Charge))

    Service_Charge.set(Service)
    cost.set(costofmeal)
    Tax.set(PaidTax)
    Subtotal.set(SubCost)
    Total.set(OverAllCost)

    row=[name,cof,colfries,cob,cofi,coic,codr,(PayTax + Totalcost + Ser_Charge)]
...
lblwings = Label(f1, font=( 'aria' ,16, 'bold' ),text="Wings 2pc",fg="steel blue",bd=10,anchor='w')
lblwings.grid(row=2,column=0)
txtwings = Entry(f1,font=('ariel' ,16,'bold'), textvariable=wings , bd=6,insertwidth=4,bg="powder blue" ,justify='right')
txtwings.grid(row=2,column=1)
...
txtice_cream = Entry(f1,font=('ariel' ,16,'bold'), textvariable=ice_cream , bd=6,insertwidth=4,bg="powder blue" ,justify='right')
...
def price():
    ...
    lblinfo = Label(roo, font=('aria', 15, 'bold'), text="Wings 2 pcs", fg="steel blue", anchor=W)
